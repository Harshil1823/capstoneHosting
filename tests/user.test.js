...
describe('updateAdminUserController', () => {
    ...
    it('should flash error and redirect if user company does not match', async () => {
        // Simulate found user from a different company
        User.findById.mockResolvedValue({ _id: 'user123', company: 'CompanyB' });
        
        await updateAdminUserController(req, res, next);
        
        expect(req.flash).toHaveBeenCalledWith('error', 'Access denied: User not in your company');
        expect(res.redirect).toHaveBeenCalledWith('/admin/users');
    });
    it('should flash error and redirect for invalid email format', async () => {
        req.body.workEmail = 'invalid-email';
        // Simulate found user with same company
        User.findById.mockResolvedValue({ _id: 'user123', company: req.user.company });
        
        await updateAdminUserController(req, res, next);
        
        expect(req.flash).toHaveBeenCalledWith('error', 'Invalid email format');
        expect(res.redirect).toHaveBeenCalledWith(`/admin/users/user123`);
    });
    it('should flash error and redirect if email is already in use', async () => {
        // Simulate found user with same company
        User.findById.mockResolvedValue({ _id: 'user123', company: req.user.company });
        // Simulate another user with the same email
        User.findOne.mockResolvedValue({ _id: 'otherUser' });
    ...
    it('should update user successfully without image upload', async () => {
        // Simulate no file uploaded
        req.file = undefined;
        const foundUser = { _id: 'user123', company: req.user.company };
    ...
    it('should update user successfully with image upload (deleting old image if exists)', async () => {
        // Simulate a found user with an old profile image
        const foundUser = { 
            _id: 'user123', 
            company: req.user.company, 
            profileImage: { filename: 'oldImage' }
        ...
        // Check that cloudinary destroy was called for the old image
        expect(cloudinary.uploader.destroy).toHaveBeenCalledWith('oldImage');
        expect(User.findByIdAndUpdate).toHaveBeenCalledWith(
            'user123',
            {
    ...
    it('should update user without changing role if provided role is invalid', async () => {
        ...
        req.file = undefined; // Remove file upload
        const foundUser = { _id: 'user123', company: req.user.company };
        User.findById.mockResolvedValue(foundUser);
        User.findOne.mockResolvedValue(null);
        User.findByIdAndUpdate.mockResolvedValue(foundUser);
        ...
        // Role should not be included in the updateData and no profileImage field expected
        expect(User.findByIdAndUpdate).toHaveBeenCalledWith(
            'user123',
            {
                firstName: req.body.firstName,
